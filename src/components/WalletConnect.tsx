...
export function WalletConnect() {
  ...
  // Fixed: Filter for available connectors properly
  const availableNames = ['WalletConnect', 'Coinbase Wallet'];
        ...
        {connectors
          ...
          .map((connector) => {
            const isAvailable = availableNames.some(name => 
              connector.name.includes(name)
            );
            
            return (
              <button
                key={connector.id}
                onClick={() => isAvailable ? connect({ connector }) : null}
